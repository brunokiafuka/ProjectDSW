		...
		<!--Section that will display the data acordding to the switch()-->
			<?php
				...
				if(isset($_GET['moviegenre'])){	
						...
						$sql=$conn->prepare("SELECT COUNT(movie_id) FROM movies WHERE movie_genre = :genre");
					...
					$q3=" SELECT * from movies where movie_genre = :genre  $limit " ;
					...
					//show user what page their on and total num of pages
						...
						if ($last != 1) {
							...
							if ($pagenum > 1) {
								$prev = $pagenum - 1;
								$pnCtrls .= '<a href="'.$_SERVER['PHP_SELF'].'?moviegenre='.$genre.'&pn='.$prev.'"><span><i class="fa fa-step-backward" aria-hidden="true"></i></span></a>&nbsp;';
								// render clickable num links that shoul appear on the left
								for ($i=$pagenum - 4; $i < $pagenum; $i++) { 
									if ($i > 0) {
										$pnCtrls .= '<a href="'.$_SERVER['PHP_SELF'].'?moviegenre='.$genre.'&pn='.$i.'"><span>'.$i.'</span></a>&nbsp;';						
									}
								}
							}
							...
							if ($pagenum != $last) { //render next value
								$next = $pagenum + 1;
								$pnCtrls .= '<a href="'.$_SERVER['PHP_SELF'].'?moviegenre='.$genre.'&pn='.$next.'"><span><i class="fa fa-step-forward" aria-hidden="true"></i></span></a>&nbsp;';
							}							
						}

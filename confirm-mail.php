		...
		<section class="login-sec cart-sec">
			...
			<?php
				...
				if (isset($_GET['id']) && isset($_GET['code'])) {
					...
					$verify=$conn->prepare("SELECT * FROM customer WHERE cust_id = ? LIMIT 1");
					...
					while ($row=$verify->fetch(PDO::FETCH_ASSOC)) {
						...
						#	echo $row['conf_code'];
							...
							$update = $conn->prepare("UPDATE customer SET conf_code = :cod WHERE cust_id = :user");
							$update->bindValue(":cod", $conf_code);
							...
							<article class='product'>
								...
								<div class='content'>
									<h1>Hi ".$userName." welcome to BuntingMovies!</h1>
									<p>You have Successfully confirmed your email.<br><a href='login.php'>Click here to login</a></p>			
								</div>	
					...
					<article class='product'>
						...
						<div class='content'>
							<h1>Hi ".$_SESSION['username']." congratulations!</h1>
							<p>You have Successfully registered and an activation email has been sent to your email. Click the activation link to activate you account.</p>			
						</div>	
